...
function love.load(args, unfilteredArgs)
    ...
    player.width = 126
    player.height = 113
    player.sprite = love.graphics.newImage("data/viking.png")
    player.lives = 5
    ...
    enemyDebris = love.graphics.newImage("data/eDmg.png")
    playerDebris = love.graphics.newImage("data/pDmg.png")
    ...
    lBorder = love.graphics.newImage("data/lbar.png")
    rBorder = love.graphics.newImage("data/rbar.png")

    threatBarSize = 0
    ltBar = love.graphics.newQuad(0, 0, 0, 0, lBorder)
    rtBar = love.graphics.newQuad(0, 0, 0, 0, rBorder)

    hbar = love.graphics.newImage("data/hbar5.png")
    p2bar = love.graphics.newImage("data/p2.png")

    score = 0
    score_file = io.open("data/hs.dat", "r")
    io.input(score_file)
    highScore = tonumber(io.read())
    io.close(score_file)

    startSound = love.audio.newSource("data/start.wav", "static")
    deathSound = love.audio.newSource("data/death.wav", "static")
    playerHitSound = love.audio.newSource("data/phit.wav", "static")
    enemyHitSound = love.audio.newSource("data/ehit.wav", "static")
    playerLaserSound = love.audio.newSource("data/plaser.wav", "static")
    enemyLaserSound = love.audio.newSource("data/elaser.wav", "static")
    music = love.audio.newSource("data/music.mp3", "stream")
    ...
    menu = love.graphics.newImage("data/menu.png")
    gameOverScreen = love.graphics.newImage("data/gameover.jpg")
...
function love.update(dt)
    if gameStart == true then
        ...
        if (player.lives < 0) or (threatLevel >= 100) then
            ...
            if score > highScore then
                highScore = score
                score_file = io.open("data/hs.dat", "w")
    ...
    elseif love.keyboard.isDown("return") or joystick:isGamepadDown("start") then 
        startSound:play()
        gameStart = true 
        gameOver = false
    end 
...
function spawnEnemy(type, x, y)
    ...
    --mutalisk
    if type == 1 then 
        enemy.sprite = love.graphics.newImage("data/mutalisk.png")
        enemy.width = 50
        enemy.height = 50
        enemy.speed = 250
        enemy.shotSpeed = 325
    ...
    --overlord
    elseif type == 2 then 
        enemy.sprite = love.graphics.newImage("data/overlord.png")
        enemy.width = 58
        enemy.height = 78
        enemy.speed = 300
        enemy.shotSpeed = 375
                ...
                (v.y > player.y - v.size + 40) and (v.y < (player.y + (player.height/2) + v.size)) then
                    if player.iframeTimer <= 0 then
                        ...
                        if player.lives >= 0 then 
                            hbar = love.graphics.newImage("data/hbar" .. player.lives ..".png") 
                        end
